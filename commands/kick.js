...
module.exports = {
    ...
    execute(msg, args, guildData, Prefix, client, Discord) {
        ...
        .then(() => { 
            msg.guild.channels.cache.get(guildData.logChannel).send(`${msg.author.tag} attempted to kick ${kickMember.tag} in #${msg.channel.name} although the user does not have permissions!`)
            .catch(err => {
        ...
        memberWithinServer.kick(`${msg.author.tag} used the kick command in #${msg.channel.name}`)
            .then(() => {
                ...
                msg.guild.channels.cache.get(guildData.logChannel).send(`${msg.author.tag} kicked ${memberWithinServer.user.tag} in #${msg.channel.name}!`)
            })
            .catch(err => {
